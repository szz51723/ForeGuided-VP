...
def load_and_split_data(filepath, lookback, horizon, batch_size, start_row=2):
    ...
    df_ext[numeric_cols] = df_ext[numeric_cols].apply(
    ...
    train_spatial = np.concatenate(
        [S[1][:n_train].reshape(-1, 2), T[1][:n_train].reshape(-1, 2)],
        axis=0
    )
    ...
    def to_grid(spatial):
        norm = np.clip(
            (spatial - [lon_min, lat_min]) / [lon_range, lat_range],
            0.0, 1.0
        )
    ...
    train_kin = np.concatenate(
        [S[3][:n_train].reshape(-1, 2), T[3][:n_train].reshape(-1, 2)],
        axis=0
    )
    kin_m, kin_s = train_kin.mean(axis=0), np.maximum(train_kin.std(axis=0), 1e-6)
    S[3], T[3] = (S[3] - kin_m) / kin_s, (T[3] - kin_m) / kin_s

    train_bat = np.concatenate([
        S[0][:n_train].reshape(-1, 1),
        T[0][:n_train].reshape(-1, 1),
        Y[:n_train].reshape(-1, 1)
    ])
...
def fit_model(name, model, train_loader, val_loader, args, v_mean, v_std, is_teacher=False, teacher_model=None):
    ...
    print(f'\n--- Training {name.capitalize()} [{args.exp_variant}] ---')
    opt = torch.optim.Adam(
        model.parameters(),
        lr=DEFAULT_MODEL_HPARAMS[name]['lr'],
        weight_decay=DEFAULT_MODEL_HPARAMS[name]['weight_decay']
    )
    ...
    for epoch in range(args.epochs):
        ...
        if args.exp_variant == 'full':
            alpha_f = get_feature_kd_alpha(
                epoch,
                args.alpha,
                args.feature_kd_warmup_epochs,
                args.feature_kd_anneal_epochs
            ) if teacher_model else 0.0
        elif args.exp_variant == 'wo_ig':
            alpha_f = args.fixed_alpha if teacher_model else 0.0
        else:
            alpha_f = 0.0
        ...
        for batch in train_loader:
            ...
            use_guidance = (
                teacher_model is not None and
                args.exp_variant in ['full', 'wo_ig']
            )
        ...
        print(
            f'[{name.capitalize()}][{args.exp_variant}] '
            f'Epoch {epoch+1}/{args.epochs}, '
            f'Train MSE: {train_mse_sum / count:.6f}, '
            f'Val MSE: {val_mse_sum / v_count:.6f}'
        )

        if stop.step(val_loss, model):
            print(f'[{name.capitalize()}][{args.exp_variant}] Early stopping at epoch {epoch+1}')
            break
...
def train_model(args):
    ...
    exp_tag = getattr(args, 'exp_variant', 'full')
    plot_dir = f'plots_{model_tag}_{exp_tag}'

    for m in ['teacher', 'student', 'baseline']:
        os.makedirs(f'{plot_dir}/{m}/voltage', exist_ok=True)

    train_ld, val_ld, test_ld, v_mean, v_std, vocab_size = load_and_split_data(
        filepath, args.lookback, args.horizon, args.batch_size
    )
    ...
    if args.exp_variant in ['full', 'wo_ig']:
    ...
    if args.exp_variant == 'baseline_only':
        student = None
        s_train_loss, s_val_loss = [], []
    else:
    ...
    if teacher is not None:
        loss_dict['teacher'] = (t_train_loss, t_val_loss)
    if args.exp_variant != 'baseline_only':
        loss_dict['student'] = (s_train_loss, s_val_loss)
    loss_dict['baseline'] = (b_train_loss, b_val_loss)

    for name, (tr_l, val_l) in loss_dict.items():
        save_path = f'{plot_dir}/{name}/voltage/loss_curve.png'
        save_loss_curve(tr_l, val_l, save_path, f'{name.capitalize()} Loss Curve [{args.exp_variant}]')

    summary_rows = []

    eval_models = []
    if teacher is not None:
        eval_models.append((teacher, 'teacher', 'blue', True))
    if args.exp_variant != 'baseline_only':
        eval_models.append((student, 'student', 'green', False))
    eval_models.append((baseline, 'baseline', 'red', False))

    for model, name, color, is_t in eval_models:
        print(f'\n--- {name.capitalize()} Real Metrics [{args.exp_variant}] ---')
        ...
        for split_name, loader in [
            ...
            save_plot(
                p_trues, p_preds,
                f'{name.capitalize()} {split_name} [{args.exp_variant}]',
                f'{base_path}_pred_vs_true.png',
                color
            )
            save_plot(
                p_trues, p_preds,
                f'{name.capitalize()} {split_name} Error [{args.exp_variant}]',
                f'{base_path}_error.png',
                color,
                is_error=True
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default='full',
        choices=['full', 'wo_ig', 'wo_guidance', 'baseline_only']
    )

    args = parser.parse_args()

    # 只对 LSTM 和 BiGRU 做消融
    target_models = ['LSTM', 'BiGRU']
    exp_variants = ['full', 'wo_ig', 'wo_guidance']
    ...
    for model_name in target_models:
        for variant in exp_variants:
            ...
            args.exp_variant = variant

            print(f'\n{"=" * 20} Running {model_name} [{variant}] {"=" * 20}\n')
            start_time = time.time()

            try:
                summary_rows = train_model(args)
                elapsed = time.time() - start_time
                print(f'[OK] {model_name} [{variant}] finished in {elapsed:.2f} s')
                ...
                elapsed = time.time() - start_time
                print(f'[ERROR] {model_name} [{variant}] failed after {elapsed:.2f} s: {e}')
                failed_models.append(f'{model_name}[{variant}]')
    ...
    if len(all_summary_rows) > 0:
        ...
        variant_order = {'full': 0, 'wo_ig': 1, 'wo_guidance': 2, 'baseline_only': 3}
        model_order = {'LSTM': 0, 'BiGRU': 1}

        summary_df['ModelOrder'] = summary_df['ModelFamily'].map(model_order)
        summary_df['VariantOrder'] = summary_df['ExpVariant'].map(variant_order)
        ...
        summary_df = summary_df.sort_values(
            by=['ModelOrder', 'VariantOrder', 'SubModelOrder', 'SplitOrder']
        ).drop(columns=['ModelOrder', 'VariantOrder', 'SubModelOrder', 'SplitOrder'])
